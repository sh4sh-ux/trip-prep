...
## 현재 버전
**v0.38** (2026-08-07) — **앱 아이콘 교체**: 알루미늄 캐리어 사진(`icons/Carrier.jpg`)을 흰 배경에 얹어 4개 크기(favicon 64 / icon-192 / icon-512 / apple-touch-icon 180) 재생성. 캐리어는 중앙 안전영역(높이 72%) 안에 배치해 maskable 대응. `sw.js` CACHE·`manifest`는 기존 경로 그대로.

### v0.37
오늘 탭 `떠나기 전 · 숙소 체크` 카드(`quickCheckCard`, 'ret')의 🧳 아이콘도 `CARRIER_ICON`(알루미늄 캐리어 사진)으로 교체. '출발 전 집 체크'(dep)는 🏠 유지

### v0.36
① `내 기본 준비물 관리` 아이콘을 실제 알루미늄 캐리어 사진으로 교체(`CARRIER_ICON` = 투명 PNG base64 내장, 원본 `icons/Carrier.jpg`). ② `순서 바꾸기` 버튼의 ↕ 유니코드를 인라인 SVG(`ARR`)로 교체 — 기기별 렌더 편차 제거, `<span style="inline-flex">`로 한 줄 고정. ③ 삭제 확인: 준비물·장보기·일정 항목 × 삭제 시 `confirm()` 후 삭제(되돌리기 토스트는 유지)

### v0.35
여행 유형에서 '출장'(biz) 제거("출장은 여행이 아니다"). `TYPES`에서 삭제 + biz 전용 준비물(`노트북·충전기`,`명함`) 정리 + 혼합 `only`(`자동차키`,`차량 거치대`)에서 biz 제거 + `migrate()`에 기존 type/packTemplateType 'biz'→'domestic' 마이그레이션
...
### v0.32
재료 분류 칩 토글 취소: 이미 선택된 분류를 다시 누르면 취소되고 자동 분류로 복귀

### v0.31
메뉴 고르기 정리: 담은 메뉴 위로 모으고, 안 담은 메뉴는 `담을 메뉴 더 보기 N개`로 접기
...
- **장보기** — `메뉴 고르기`(행 리스트: ○ 담기 · 이름 눌러 묶음 보기) / `장볼 것`(매대별 아코디언 + 전체 접기/펼치기 + 순서 바꾸기)
  - **장볼 것 순서 바꾸기(v0.33)** — `↕ 순서 바꾸기` 버튼(`#btnMktSort`, `UI.mktSort`)으로 진입. 준비물과 동일한 드래그(`.drag-h`): 매대(섹션) 순서는 전역 `DB.mktOrder`(마트 동선용), 매대를 탭해 열면 안의 항목 순서는 여행별 `t.buyOrder`(키 목록). 항목은 파생 데이터라 순서에 없던 새 항목은 뒤로·사라진 항목은 자동 정리. 표시는 `orderedMktCats()`·`buyRank(t)`가 담당
...
1. ~~아이콘 교체~~ — **v0.38에서 완료.** 사용자가 올린 알루미늄 캐리어 사진(`icons/Carrier.jpg`)을 흰 배경 앱 아이콘으로 사용. (원래 "캐리어+비행기" 구상 → 실제 제품 사진으로 대체)
