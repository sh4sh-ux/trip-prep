...
## 현재 버전
**v0.36** (2026-08-07) — ① `내 기본 준비물 관리` 아이콘을 실제 알루미늄 캐리어 사진으로 교체(`CARRIER_ICON` = 투명 PNG base64 내장, 원본 `icons/Carrier.jpg`). ② `순서 바꾸기` 버튼의 ↕ 유니코드를 인라인 SVG(`ARR`)로 교체 — 기기별 렌더 편차 제거, `<span style="inline-flex">`로 한 줄 고정. ③ 삭제 확인: 준비물·장보기·일정 항목 × 삭제 시 `confirm()` 후 삭제(되돌리기 토스트는 유지)
